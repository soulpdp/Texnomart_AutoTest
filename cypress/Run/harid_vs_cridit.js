...
import Harid_qilish_pom from "../POM/Harid_qilish_pom";

function run_all_logic() {
    cy.fixture("mudatli_tolov_assertion.json").then((data) => {
        const credit = new Mudatli_tolov_pom_TEXNOMART();
        const buy = new Harid_qilish_pom();
        ...
        buy.to_basket();
        buy.enter_basket();
        buy.final_assertion_check(data.check_1);
        buy.tolov_turi_btn()
        buy.tolov_turi_payme();
        buy.tolov_turi_click_submit();
        buy.dokon_tanlash_button();
        buy.dokon_tanlash_chose();
        buy.final_submit();
